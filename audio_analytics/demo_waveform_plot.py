...
def get_waveform_plot(filename, samplerate=0, block_size=4096, ax=None, downsample=2 ** 4):
    ...
    downsample = 1  # downsample  # to plot n samples / hop_s

    a = source(filename, samplerate, hop_s)  # source file
    if samplerate == 0: samplerate = 44100  # a.samplerate
...
def set_xlabels_sample2time(ax, latest_sample, samplerate):
    ...
    if latest_sample / float(samplerate) > 60:
        ...
        ax.set_xticklabels(
            ["%02d:%02d" % (t / float(samplerate) / 60, (t / float(samplerate)) % 60) for t in ax.get_xticks()[:-1]],
            rotation=50)
    else:
        ax.set_xlabel('time (ss.mm)')
        ax.set_xticklabels(
            ["%02d.%02d" % (t / float(samplerate), 100 * ((t / float(samplerate)) % 1)) for t in ax.get_xticks()[:-1]],
            rotation=50)
